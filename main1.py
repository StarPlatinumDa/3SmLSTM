...
def ema_update(source, target, decay=0.99, start_itr=20, itr=None):
    ...
    # 如果提供了迭代计数器并且itr小于起始itr，将ema权重与基础权重挂钩。
...
def init_seed(seed):
    ...
    # torch.backends.cudnn.enabled = True  #默认开启
    ...
    # 设置 CUDNN 为非基准测试模式。基准测试模式（True）允许 CUDNN 自动选择最优的算法来提高性能，但这可能会导致每次运行的结果不同。
    # 设置为 False 可以确保使用一致的算法，从而保证结果的一致性。
...
class Processor():
    ...
    def train(self, epoch, save_model=False):
        ...
        temp_lr = self.adjust_learning_rate(epoch)
        ...
        self.print_log(
            '\tMean training loss: {:.4f}. lr: {:.4f}.  Mean training acc: {:.2f}%.'.format(np.mean(loss_value),
                                                                                            temp_lr,
